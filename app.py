...
app = Flask(__name__)
CORS(app, resources={r"/predict": {"origins": ["http://localhost:3000", "http://192.168.1.3:3000", "https://kamaibisubilar.vercel.app"]}})
...
def extract_keypoints(results):
    ...
    additional_features = np.zeros(18)  # 18 additional features
...
try:
    if os.path.exists(min_val_path) and os.path.exists(max_val_path):
        ...
        expected_keypoint_dim = 33*4 + 21*3 + 21*3 + 18  # 276
        if min_val.shape[0] != expected_keypoint_dim:
            logger.error(f"Min/max shape mismatch: {min_val.shape[0]} (expected {expected_keypoint_dim})")
            raise ValueError(f"Min/max shape mismatch: {min_val.shape[0]} (expected {expected_keypoint_dim})")
    else:
    ...
    keypoint_dim = 33*4 + 21*3 + 21*3 + 18  # 276
...
try:
    if os.path.exists(model_path):
        ...
        expected_input_shape = (1, sequence_length, 33*4 + 21*3 + 21*3 + 18)  # (1, 30, 276)
